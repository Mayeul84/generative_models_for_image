...
class DDPM(SpacedDiffusion):
    ...
    def diffuse_back(self, model, x, t_start=0, t_end=None):
        if not t_end:
            t_end = self.num_timesteps
        assert t_start < t_end
        pbar = list(range(self.num_timesteps))[::-1][t_start:t_end]
        for idx in pbar:
            time = torch.tensor([idx] * x.shape[0], device = x.device)
            out = self.p_sample(x=x, t=time, model=model)
            x = out['sample'].detach_()
